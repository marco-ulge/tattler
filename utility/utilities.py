...
def concatenate_char(char, quantity):
    """Concatenates the specified char for quantity amount

    Arguments:
        char {string} -- the text to be repeated
        quantity {int} -- the amount of time to concatenate char with itself

    Returns:
        to_print -- the concatenated string
    """
    to_print = ""
    to_print += char * quantity
    return to_print
...
def does_exist(path):
    ...
    try:
        if not os.path.exists(path):
            raise Exception("The specified repository does not exist")
